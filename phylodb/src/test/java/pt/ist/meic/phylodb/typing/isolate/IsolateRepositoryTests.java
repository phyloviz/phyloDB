...
public class IsolateRepositoryTests extends RepositoryTests {
	...
	private static Isolate isolate(String id, long version, boolean deprecated, String description, Ancillary[] ancillary, Profile profile) {
		...
		return new Isolate(project1.getPrimaryKey(), dataset.getPrimaryKey().getId(), id, version, deprecated, description, ancillary, reference);
	}
	...
	private static Stream<Arguments> find_params() {
		...
		Isolate.PrimaryKey key = new Isolate.PrimaryKey(project1.getPrimaryKey(), dataset.getPrimaryKey().getId(), "3");
	...
	public void findAll(int page, Isolate[] state, Isolate[] expected) {
		store(state);
		Optional<List<Isolate>> result = isolateRepository.findAll(page, LIMIT, project1.getPrimaryKey(), dataset.getPrimaryKey().getId());
	...
	public void saveAll(List<Isolate> isolates, Isolate[] state, Isolate[] expectedState, boolean executed, int nodesCreated, int relationshipsCreated) {
		store(state);
		Optional<QueryStatistics> result = isolateRepository.saveAll(isolates, project1.getPrimaryKey().toString(), dataset.getPrimaryKey().getId().toString());
		if (executed) {
			assertTrue(result.isPresent());
			assertEquals(nodesCreated, result.get().getNodesCreated());
			assertEquals(relationshipsCreated, result.get().getRelationshipsCreated());
