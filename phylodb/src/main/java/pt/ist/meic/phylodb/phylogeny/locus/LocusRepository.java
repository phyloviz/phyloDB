...
public class LocusRepository extends EntityRepository<Locus, Locus.PrimaryKey> {
	...
	protected List<Locus> getAll(int page, int limit, Object... filters) {
		if (filters == null || filters.length == 0)
			return null;
		String statement = "MATCH (t:Taxon {id: $})-[:CONTAINS]->(l:Locus)\n" +
	...
	protected void create(Locus locus) {
		String statement = "MATCH (t:Taxon {id: $})\n" +
				...
				"CREATE (t)-[:CONTAINS]->(l:Locus {id: $, description: $, from: datetime()})";
	...
	protected void update(Locus locus) {
		String statement = "MATCH (t:Taxon {id: $})-[:CONTAINS]->(l:Locus {id: $})\n" +
				...
				"SET output.description = $, output.from = datetime(), l.to = datetime() WITH output\n" +
				"MATCH (s:Schema)-[r:HAS]->(output) DELETE r";
		execute(new Query(statement, locus.getTaxonId(), locus.getId(), locus.getDescription()));
	}
